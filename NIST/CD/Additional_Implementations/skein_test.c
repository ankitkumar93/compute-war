...
uint_t Skein_Test(uint_t blkSize,uint_t maxLen,uint_t hashLen,uint_t nStep)
    {
    enum        { MAX_BUF=1024 };
...
void ShowCompiler(const char *CVER)
    {
    printf(" //: %2u-bit, %s%s",(uint_t)(8*sizeof(size_t)),COMPILER_ID,CVER);
            ...
            if (blkSize & i)
                ...
                if (hashLen == 0)              /* use all hash sizes? */
                    ...
                    for (n=0;n < HASH_BITS_CNT;n++)
                        testCnt += Skein_Test(i,maxLen,HASH_BITS[n],0);
                    }
                else
                    testCnt += Skein_Test(i,maxLen,hashLen,0);
                }
            }
        if (testCnt)
